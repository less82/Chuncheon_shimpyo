...
- **`stops.json`**: 정류장 1,890개. 내부 마스터로 사용한다.
- **TAGO 노드 ID**: 1,845개. 45개가 미매핑 상태다.
- **경유 노선 보유 정류장**: 1,853개. 37개가 미연결 상태다.
- **`routes.json`**: 223개 노선과 정류장 참조 16,168건. 깨진 참조는 0건이다.
- **수요 ID 매핑**: 1,038건. 직접 49건, 추론 15건, 미해결 974건이다.
...
- **장소 검색**
  - 운영 선택: 카카오 Local 등 국내 API
  - 현재 구현: Nominatim 공개 서버
  - 결론: 서버 어댑터로 교체
- **대중교통 경로**
  - 운영 선택: ODsay 등 경로 API
  - 현재 구현: 로컬 직행 탐색
  - 결론: 운영 API 필요
- **실시간 도착**
  - 운영 선택: TAGO
  ...
  - 결론: 서버 프록시 필수
- **도보 경로**
  - 운영 선택: 계약 API 또는 자체 OSRM
  - 현재 구현: OSRM 공개 데모
  - 결론: 운영 사용 금지
- **음성 인식**
  - 운영 선택: 브라우저 STT와 직접 입력
  - 현재 구현: Web Speech
  - 결론: 보조 입력으로만 사용
